  ...
  const onChangePage = debounce(
    useCallback(
      (
        ...
        if (loading || config?.role === "viewer" || following?.length) return;
        ...
        const BindingStore = plugin.stores.create(
          `${page.id}-bindings`,
          storeConf
        );
      ...
      [loading, activeTool, page, config, following]
    ),
    250
  );

...
const MainProvider = ({ children }: { children: any }) => {
  ...
  useEffect(() => {
    ...
    if (config.follow) {
      plugin.emit("requestUserPosition", [config.follow]);
    }
  }, [plugin, app, config]);
  ...
  const loadPlugin = async () => {
    ...
    const {
      payload: { roomName },
    } = await dytePlugin.room.getID();
    setMeetingId(roomName);

    // get user ID
    const {
      payload: { peer },
    } = await dytePlugin.room.getPeer();
    const {
      payload: { enabledBy },
    } = await dytePlugin.enabledBy();
  ...
  useEffect(() => {
    if (!app || !plugin) return;
    plugin.addListener("onMove", ({ user, camera, size, override }) => {
      // move user
      if (!user || !user.id) {
        setError("Could not load user.");
        return;
      ...
      if (!(followID || followID == user.metadata.id || override)) return;
      ...
      if (size && (window.innerHeight < size.y || window.innerWidth < size.x)) {
        ...
        if (camera) {
          app.setCamera(
            [camera.point[0] / maxRatio, camera.point[1] / maxRatio],
            viewerZoom,
            "follow"
          );
        }
